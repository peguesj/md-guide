...
- For old PlantUML versions, you have to use `skinparam useBetaStyle true`.
- For new PlantUML versions, `style` is enabled by default.

> [Skinparam](skinparam) is being phased out (see [comments in issue#1464](https://github.com/plantuml/plantuml/issues/1464#issuecomment-1601464639)) and users should migrate to `style`.
...
[skinparam](skinparam) can change the rendering of diagrams (fonts, colors...):
...
* BackGroundColor
* HyperLinkColor
* LineColor _(old ~~BorderColor~~)_ : seems more generic
* LineThickness
* LineStyle _(old ~~BorderStyle~~)_: on real number or two real numbers separated with ``-``
* Padding
* Margin
* RoundCorner
